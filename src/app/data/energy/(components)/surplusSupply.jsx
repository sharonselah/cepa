...
const AfricaMapSurplusSupply = () => {
    ...
    useEffect(() => {
        d3.json("https://raw.githubusercontent.com/janasayantan/datageojson/master/geoAfrica.json")
            .then((data) => {
                ...
                // Generate surplus supply data (0-1000 MW)
                const surplusSupplyData = {};
                const countries = [];
    
                data.features.forEach((d) => {
                    const countryName = d.properties.name;
                    countries.push(countryName);
                    surplusSupplyData[countryName] = {
                        surplusSupply: Math.random() * 1000, // Random value between 0 and 1000 MW
                    };
                });
    
                setCountryData(surplusSupplyData);
                setCountryList(countries.sort());
    
                // Render the map
                renderMap(data, surplusSupplyData);
    ...
    const renderMap = (geoData, dataByCountry) => {
        ...
        // Define surplus supply color scale (Green)
        const colorScale = d3.scaleSequential(d3.interpolateGreens).domain([0, 1000]);
        ...
        svg.append("g")
            ...
            .attr("d", path)
            .attr("fill", (d) => {
                const surplusSupplyValue = dataByCountry[d.properties.name]?.surplusSupply || 0;
                return colorScale(surplusSupplyValue);
            })
            .attr("stroke", "#fff")
            .attr("stroke-width", 1)
            .on("mouseover", function (event, d) {
                ...
                const surplusSupplyValue = dataByCountry[country]?.surplusSupply.toFixed(2) || "N/A";
    
                const [x, y] = path.centroid(d);
                const bounds = containerRef.current.getBoundingClientRect();
    
                tooltip.classed("opacity-100", true)
                    .html(`<strong style="color: #006400; font-size: 18px;">${country}</strong><br><hr>
                           <div style="padding: 4px 0;">Surplus Supply: ${surplusSupplyValue} MW</div>`)
